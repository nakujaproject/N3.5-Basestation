...
int = 50;
id = X(1:int:1000,2); % also accounts for time in  millis
altitude = X(1:int:1000, 10);
pressure = X(1:int:1000, 12);
velocity = X(1:int:1000, 11);
...
simulated_parameter = pressure;  % data with noise
...
figure;
plot(1:rows, filtered_parameter, '-b', 1:rows, pressure, '-r')
xlabel('Time Step');
ylabel('Pressure');
legend('filtered pressure', 'raw pressure')
title('Kalman Filtered Pressure');
grid on;